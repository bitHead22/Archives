...
export default function SelectCoursePage() {
  ...
  if (courses && courses.length > 0) {
    matchedCourse = courses.find(c => {
      ...
      // e.g. "b.tech" and "cse" in "b.tech cse"
      return name.includes(selectedDegree.id.toLowerCase()) && name.includes(selectedBranch.id.toLowerCase())
    })
    // Fallback so the UI is clickable even if database doesn't have the exact selected combo
    if (!matchedCourse) {
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            <nav className="hidden md:flex items-center gap-6">
              ...
              <a className="text-[#a3a3a3] hover:text-white transition-colors text-sm font-medium" href="#">About</a>
              <a className="text-[#a3a3a3] hover:text-white transition-colors text-sm font-medium" href="#">Contact</a>
            </nav>
                ...
                {DEGREES.map((deg) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`group relative flex flex-col items-center p-6 bg-[#000000] rounded-sm border cursor-pointer transition-all ${isSelected ? 'border-white' : 'border-[#262626] hover:border-neutral-600'}`}
                    >
                      {isSelected && (
                      ...
                      <div className={`w-12 h-12 mb-4 rounded-sm flex items-center justify-center transition-colors border border-neutral-800 ${isSelected ? 'bg-white text-black' : 'bg-neutral-900 text-neutral-400 group-hover:bg-white group-hover:text-black'}`}>
                        <Icon size={24} />
                      </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {filteredBranches.map((branch) => {
                  ...
                  return (
                    <label key={branch.id} className="cursor-pointer relative group">
                      <input 
                        type="radio" 
                        name="branch" 
                      ...
                      />
                      <div className={`flex items-center p-3 bg-black border rounded-sm transition-all ${isSelected ? 'border-white bg-[#171717]' : 'border-neutral-800 hover:border-neutral-600'}`}>
                        <div className={`p-1.5 rounded-sm mr-3 border transition-colors ${isSelected ? 'bg-neutral-900 text-white border-neutral-800' : 'bg-neutral-900 text-neutral-400 border-neutral-800 group-hover:text-white'}`}>
                          <Icon size={18} />
                        </div>
                        <div>
                          <p className={`font-medium text-sm ${isSelected ? 'text-white' : 'text-neutral-300 group-hover:text-white'}`}>{branch.title}</p>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="bg-neutral-900/50 border border-neutral-800 rounded-sm p-3 flex gap-3 items-start">
                    ...
                    <p className="text-xs text-neutral-400 leading-relaxed">
                      AI Assistant ready for <span className="text-white font-medium">{selectedDegree.title} {selectedBranch.title}</span> papers.
                    </p>
                  </div>
                </div>
              ...
              <div className="p-4 rounded-sm border border-dashed border-neutral-800">
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <a className="text-xs font-medium text-white hover:text-neutral-300 transition-colors underline decoration-neutral-600 underline-offset-2" href="#">Request addition</a>
                  </div>
                </div>
              </div>
            </div>
